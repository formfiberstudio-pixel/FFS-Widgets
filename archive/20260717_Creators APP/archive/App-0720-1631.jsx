...
function App() {
  ...
  // Trackers tracking toggled/collapsed sections
  ...
  // Bulk Category Selection Actions
  ...
  const handleCollapseAllCategories = () => {
    const allTitles = [];
    Object.values(groupedProjects).forEach(projs => {
      projs.forEach(p => {
        if (p.title) allTitles.push(p.title);
      });
    });
    setSelectedProjectFilters(allTitles);
  };
  ...
  // -------------------------------------------------------------
  // 4. TIMELINE SLOTS & CAROUSEL UTILITIES
  // -------------------------------------------------------------
  let slots = [];
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 p-8 ${
      ...
      <div className="max-w-[1600px] mx-auto">
        ...
        <header className="mb-6 flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            {/* Organized Selector Order Fix: Year, Month, Week */}
        ...
        <div className={`grid grid-cols-1 ${isSidebarOpen ? 'lg:grid-cols-12' : ''} gap-8 items-start`}>
          ...
          {isSidebarOpen && (
            <div className={`lg:col-span-3 p-5 rounded-xl border flex flex-col bg-white border-slate-200 shadow-xs`} style={{ borderRadius: `${cardRadius}px` }}>
              <div className="mb-4">
                ...
                {/* 1. Expand All & Collapse All Controls Insertion */}
                <div className="flex items-center gap-2 mb-2 pb-2 border-b border-slate-100">
                  <button 
                    onClick={handleExpandAllCategories}
                    className="text-[10px] font-bold text-slate-500 hover:text-rose-500 bg-slate-100 hover:bg-rose-50 px-2 py-1 rounded transition-colors cursor-pointer"
                  >
                    Expand All
                  </button>
                  <button 
              ...
              <div className="space-y-3 max-h-[68vh] overflow-y-auto pr-1">
                {Object.entries(groupedProjects).map(([type, projs]) => (
                  <div key={type} className="border border-slate-200 rounded-md p-2 bg-slate-50/50">
                    ...
                    <div className="space-y-1.5">
                      {projs.map((p, i) => {
                        const isCollapsed = selectedProjectFilters.includes(p.title);
                        return (
                          <div 
                            key={i} 
                            onClick={() => toggleProjectFilter(p.title)}
                            className={`text-xs p-2.5 rounded border transition-all cursor-pointer bg-white ${
                              isCollapsed ? 'border-slate-200 opacity-40 hover:opacity-70' : 'border-slate-300 font-medium text-slate-800 shadow-2xs hover:border-rose-400'
                            }`}
                          >
                            {p.title}
                          </div>
          ...
          {/* Right Main Time Series Dashboard */}
          <div className={isSidebarOpen ? 'lg:col-span-9' : 'w-full'}>
            
            {/* Structural Column Header Guide for Non-Linear Year Layouts */}
            {viewMode !== 'year' && (
            ...
            {viewMode === 'month' ? (
                  ...
                  <div key={rowIndex} className="grid w-full" style={{ gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))`, gap: `${gap}px` }}>
                    {rowSlots.map((slot, slotIndex) => {
                      ...
                      return (
                        ...
                        >
                          {/* 4. Removed opacity / dark tone mask inside calendar cards */}
                          {hasLog && pLog.imageUrl && (
                            <img src={pLog.imageUrl} className="absolute inset-0 w-full h-full object-cover z-0" alt="" />
                          )}
                          
                          {/* 3. Re-enabled color badge context circles behind indicators */}
                          <div 
              ...
              /* ----------------------------------------------------------------- */
              /* VIEW MODE: WEEKLY TIMELINE PLOT                                   */
              /* ----------------------------------------------------------------- */
                ...
                {slots.map((slot, index) => {
                  ...
                  return (
                    <div key={index} className="flex flex-col gap-2">
                      {/* 5. Fixed images and texts inside week views */}
                      <div 
                        onClick={() => slot.dateObj && setSelectedLogModal({ dateObj: slot.dateObj, logs })}
                        className={`relative aspect-[3/4] overflow-hidden p-3 border cursor-pointer flex flex-col justify-end bg-white border-slate-200 shadow-2xs ${
                          isToday(slot.dateObj) ? 'ring-2 ring-rose-500' : ''
                        }`}
                        style={{ borderRadius: `${cardRadius}px` }}
                      >
                        {hasLog && pLog.imageUrl && (
                      ...
                      {hasLog && pLog.pageContent && (
                        <div className="text-[10px] p-2 bg-white rounded border border-slate-200 text-slate-500 line-clamp-4 leading-normal shadow-3xs">
                          {pLog.pageContent}
                        </div>
                      )}
                    </div>
              ...
              /* ----------------------------------------------------------------- */
              /* VIEW MODE: YEARLY INTERACTIVE MAP (AUTOMATED MATRIX GRAPH)         */
              /* ----------------------------------------------------------------- */
              <div className={`p-6 border overflow-x-auto bg-white border-slate-200 shadow-2xs`} style={{ borderRadius: `${cardRadius}px` }}>
                <div className="min-w-[1020px] select-none">
                  ...
                  {/* Matrix Month-by-Month Iteration */}
      ...
      {/* Slide Deck Modal Dialog Box Component */}
      {selectedLogModal && (
              ...
              {selectedLogModal.logs.length > 0 ? (
                selectedLogModal.logs.map((log, idx) => (
                  <div key={idx} className="shrink-0 w-[320px] p-4 border border-slate-200 rounded-xl bg-slate-50 flex flex-col gap-3 shadow-2xs">
                    {/* 4. Keeps modal view crisp and vibrant */}
