...
const getOntarioStatHolidayName = (dateObj) => {
  ...
  // Good Friday Calculation (Meeus/Jones/Butcher algorithm)
  const a = year % 19;
...
function App() {
  // -------------------------------------------------------------
  // 1. STATE & INITIALIZATION (DYNAMICALLY TRACKS CURRENT DAY)
  // -------------------------------------------------------------
  const today = new Date(); // Uses actual system date (e.g. July 21st)
  ...
  // Hover tracking for projects & annual matrix week blocks
  const [hoveredProjectTitle, setHoveredProjectTitle] = useState(null);
  const [hoveredWeek, setHoveredWeek] = useState(null); // { mIdx, weekIndex }
  
  // Track collapsed state per Category Type
  ...
  const handleExpandAllCategories = () => {
    const freshState = {};
    Object.keys(groupedProjects).forEach(type => { freshState[type] = false; });
    setCollapsedTypes(freshState);
  };

  const handleCollapseAllCategories = () => {
    const freshState = {};
    Object.keys(groupedProjects).forEach(type => { freshState[type] = true; });
    setCollapsedTypes(freshState);
  };
  ...
  // -------------------------------------------------------------
  // 4. WEEK JUMP HANDLER FOR ANNUAL MATRIX VIEW
  // -------------------------------------------------------------
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 p-8 ${
              ...
              <div className="flex flex-col" style={{ gap: `${gap}px` }}>
                {rows.map((rowSlots, rowIndex) => (
                  <div key={rowIndex} className="flex items-stretch gap-2">
                    ...
                    {/* Weekly View Row Tab Handle */}
                    <button
                      onClick={() => {
                        const targetSlot = rowSlots.find(s => s.isValid && s.dateObj) || rowSlots[0];
                        if (targetSlot && targetSlot.dateObj) {
                          setCurrentDate(targetSlot.dateObj);
                          setViewMode('week');
                        }
                      }}
                      ...
                      {rowSlots.map((slot, slotIndex) => {
                        ...
                        return (
                          <div 
                            ...
                            onMouseEnter={() => {
                              if (hasLog && pLog) setHoveredProjectTitle(pLog.Projects || 'Untitled Project');
                            }}
                            onMouseLeave={() => setHoveredProjectTitle(null)}
                          ...
                          >
                            ...
                            <div 
                              className={`absolute top-2 left-2 w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-bold z-10 text-white shadow-2xs ${
                                isToday(slot.dateObj) && !hasLog ? 'bg-rose-500' : ''
                              }`} 
                              style={{ backgroundColor: hasLog ? getDotColor(pLog) : undefined }}
                            >
                              {slot.dayNum}
                            </div>
              ...
              <div className="grid items-start" style={{ gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))`, gap: `${gap}px` }}>
                {slots.map((slot, index) => {
                  ...
                  return (
                    <div 
                      key={index} 
                      className={`flex flex-col bg-white border border-slate-200 shadow-2xs overflow-hidden transition-all ${
                        isToday(slot.dateObj) ? 'ring-2 ring-rose-500 ring-offset-1 z-10' : ''
                      }`} 
                      style={{ borderRadius: `${cardRadius}px` }}
                    >
                      ...
                      <div className="p-3 aspect-[1/2] flex flex-col overflow-hidden">
                        {hasLog && pLog.pageContent && (
                          <p className="text-[11px] text-slate-500 leading-normal whitespace-pre-wrap line-clamp-[14]">
                            {pLog.pageContent}
                          </p>
                        )}
                      </div>
                        ...
                        return (
                          <div 
                            ...
                            className={`text-[9px] font-black tracking-tight py-1 rounded ${
                              isWeekend ? 'bg-rose-50 text-rose-600 font-bold border border-rose-100' : 'text-slate-400'
                            }`}
                          >
                        ...
                        <div key={monthLabel} className="grid grid-cols-[65px_1fr] items-center relative group">
                          ...
                          <div className="grid grid-cols-37 gap-1 items-center relative py-1">
                            ...
                            {Array.from({ length: 37 }).map((_, colIndex) => {
                              ...
                              const isWeekStart = colIndex % 7 === 0;
                              const isWeekEnd = colIndex % 7 === 6 || colIndex === 36;

                              const weekRoundClass = isWeekStart 
                                ? 'rounded-l-md -ml-1 -mr-0.5' 
                                : isWeekEnd 
                                  ? 'rounded-r-md -ml-0.5 -mr-1' 
                                  : '-mx-0.5 rounded-none';

                              const weekHighlightStyle = isHoveredWeekCell
                                ? `bg-amber-100 border-y border-amber-300/80 z-20 ${isWeekStart ? 'border-l' : ''} ${isWeekEnd ? 'border-r' : ''}`
                                : '';
                              ...
                              if (!isValidCalendarDay) {
                                return (
                                  <div 
                                    key={colIndex} 
                                    onClick={() => handleWeekClick(mIdx, weekIndex)}
                                    onMouseEnter={() => setHoveredWeek({ mIdx, weekIndex })}
                                    onMouseLeave={() => setHoveredWeek(null)}
                                    className={`h-6 flex items-center justify-center transition-colors cursor-pointer ${weekRoundClass} ${
                                      isHoveredWeekCell ? weekHighlightStyle : isWeekendColumn ? 'bg-rose-50/30 z-10' : 'z-10'
                                    }`} 
                                  />
                                );
                              }
                              ...
                              const statHolidayName = getOntarioStatHolidayName(targetDate);
                              const isStatHoliday = statHolidayName !== null;
                              ...
                              const slotBackground = isHoveredWeekCell
                                ? weekHighlightStyle
                                : isStatHoliday 
                                  ? 'bg-amber-50/40' 
                                  : isWeekend 
                              ...
                              if (hasLog) {
                                ...
                                if (isHoveredProject) {
                                  ringClass = 'ring-2 ring-rose-500/50 scale-125 z-30';
                                }

                                dotStyles = `text-white font-bold border border-transparent shadow-xs scale-110 ${ringClass}`;
                              } else if (isStatHoliday) {
                                dotStyles = 'border border-amber-200 bg-amber-50 text-amber-500 font-medium hover:border-amber-300 hover:text-amber-700';
                              } else if (isWeekend) {
                                dotStyles = 'border border-rose-200 bg-rose-50 text-rose-400 font-medium hover:border-rose-300 hover:text-rose-600';
                              }
                              ...
                              return (
                                <div
                                  ...
                                  onMouseEnter={() => {
                                    ...
                                    if (hasLog && primaryLog) setHoveredProjectTitle(primaryLog.Projects || 'Untitled Project');
                                  }}
                                  onMouseLeave={() => {
                                    setHoveredWeek(null);
                                    setHoveredProjectTitle(null);
                                  }}
                                  className={`h-6 flex items-center justify-center relative cursor-pointer group/node transition-colors ${weekRoundClass} ${slotBackground}`}
                                >
                                  <div
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      setSelectedLogModal({ dateObj: targetDate, logs });
                                    }}
                                    className={`w-4 h-4 rounded-full flex items-center justify-center text-[8px] transition-all relative z-10 ${dotStyles} ${
                                      isToday(targetDate) ? 'ring-2 ring-rose-500 ring-offset-1 font-bold' : ''
                                    }`}
                                    style={{ backgroundColor: hasLog ? currentDotHex : undefined }}
                                  >
                                    {targetDayNum}
                                  </div>
                                </div>
      ...
      {/* Detail Slide Modal Component with Stat Holiday Badge */}
      {selectedLogModal && (
