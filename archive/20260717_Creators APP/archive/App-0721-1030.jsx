...
// -------------------------------------------------------------
const isCanadianStatHoliday = (dateObj) => {
  ...
  if (month === 8 && day === 30) return true; // Truth & Reconciliation
  ...
  if (month === 4 && dayOfWeek === 1 && day >= 18 && day <= 24) return true;
  // Civic Holiday: 1st Monday of August
  if (month === 7 && dayOfWeek === 1 && day <= 7) return true;
      ...
      <div className="max-w-[1600px] mx-auto">
        ...
        <div className={`grid grid-cols-1 ${isSidebarOpen ? 'lg:grid-cols-12' : ''} gap-8 items-start`}>
          ...
          <div className={isSidebarOpen ? 'lg:col-span-9' : 'w-full'}>
            ...
            {/* ----------------------------------------------------------------- */}
            {/* VIEW MODE: MONTHLY GRID LAYOUT WITH WEEK JUMP TABS               */}
            {/* ----------------------------------------------------------------- */}
            {viewMode === 'month' ? (
                        ...
                        <div key={monthLabel} className="grid grid-cols-[65px_1fr] items-center relative group">
                          ...
                          <div className="grid grid-cols-37 gap-1 items-center relative py-1">
                            ...
                            {Array.from({ length: 37 }).map((_, colIndex) => {
                              ...
                              const isStatHoliday = isCanadianStatHoliday(targetDate);
                              const isSpecialDay = isWeekend || isStatHoliday;
                              ...
                              return (
                                <div
                                  ...
                                  className={`w-5 h-5 flex items-center justify-center relative z-10 cursor-pointer group/node rounded-xs ${
                                    isSpecialDay ? 'bg-rose-50/40' : ''
                                  }`}
                                >
                                  <div
                                    className={`w-4 h-4 rounded-full flex items-center justify-center text-[8px] transition-all ${
                                      hasLog 
                                        ? `text-white font-bold border border-transparent shadow-xs scale-110 ${isSpecialDay ? 'ring-2 ring-rose-400/50' : ''}` 
                                        : isSpecialDay
                                          ? 'border border-rose-200 bg-rose-50 text-rose-400 font-medium hover:border-rose-300 hover:text-rose-600'
                                          : 'border border-slate-200 bg-white text-slate-400 font-normal hover:border-slate-400 hover:text-slate-600'
