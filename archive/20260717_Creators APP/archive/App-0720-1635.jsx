...
function App() {
  ...
  const handleExpandAllCategories = () => {
    ...
    Object.keys(groupedProjects).forEach(type => {
      freshState[type] = false; // false means open / not collapsed
    });
  ...
  const handleCollapseAllCategories = () => {
    const freshState = {};
    Object.keys(groupedProjects).forEach(type => {
      freshState[type] = true; // true means closed / hidden
    });
    setCollapsedTypes(freshState);
  };
      ...
      <div className="max-w-[1600px] mx-auto">
        ...
        <header className="mb-6 flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            {/* Organized Button View Selection Order: Year, Month, Week */}
                    ...
                    <div key={type} className="border border-slate-200 rounded-md bg-slate-50/50 overflow-hidden">
                      ...
                      {!isHidden && (
                        <div className="p-2 pt-0 space-y-1.5 border-t border-slate-150">
                          {projs.map((p, i) => {
                            const isFiltered = selectedProjectFilters.includes(p.title);
                            return (
                              <div 
                                key={i} 
                                onClick={() => toggleProjectFilter(p.title)}
                                className={`text-xs p-2.5 rounded border transition-all cursor-pointer bg-white ${
                                  isFiltered ? 'border-slate-200 opacity-40 hover:opacity-70' : 'border-slate-300 font-medium text-slate-800 shadow-2xs hover:border-rose-400'
                                }`}
                              >
                                {p.title}
                              </div>
                ...
                {slots.map((slot, index) => {
                  ...
                  return (
                    <div key={index} className="flex flex-col bg-white border border-slate-200 shadow-2xs overflow-hidden" style={{ borderRadius: `${cardRadius}px` }}>
                      {/* Aspect Ratio Controlled Top Image Block Area */}
                      <div 
                        onClick={() => slot.dateObj && setSelectedLogModal({ dateObj: slot.dateObj, logs })}
                      ...
                      {/* Extended Text Block Flow Region Situated Beneath Card Frame Graphic */}
