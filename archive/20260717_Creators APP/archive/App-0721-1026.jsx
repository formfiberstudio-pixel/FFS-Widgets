                ...
                {slots.map((slot, index) => {
                  ...
                  return (
                    <div key={index} className="flex flex-col bg-white border border-slate-200 shadow-2xs overflow-hidden" style={{ borderRadius: `${cardRadius}px` }}>
                      ...
                      {/* Top Image Frame (matches monthly view aspect-[3/4] card ratio) */}
                      <div 
                        onClick={() => slot.dateObj && setSelectedLogModal({ dateObj: slot.dateObj, logs })}
                            ...
                            {Array.from({ length: 37 }).map((_, colIndex) => {
                              ...
                              if (!isValidCalendarDay) {
                                return (
                                  <div 
                                    key={colIndex} 
                                    className={`w-5 h-5 z-10 rounded-xs ${isWeekend ? 'bg-rose-50/30' : ''}`} 
                                  />
                                );
                              }
                                ...
                                >
                                  <div
                                    className={`w-4 h-4 rounded-full flex items-center justify-center text-[8px] font-bold border transition-all ${
                                      hasLog 
                                        ? `text-white border-transparent shadow-xs scale-110 ${isWeekend ? 'ring-2 ring-rose-400/50' : ''}` 
                                        : isWeekend
                                          ? 'border-rose-300 bg-rose-50 text-rose-600 font-extrabold hover:border-rose-400'
