                ...
                {slots.map((slot, index) => {
                  ...
                  return (
                    <div key={index} className="flex flex-col bg-white border border-slate-200 shadow-2xs overflow-hidden" style={{ borderRadius: `${cardRadius}px` }}>
                      {/* Top Image Frame (3:2 ratio) */}
                      <div 
                        onClick={() => slot.dateObj && setSelectedLogModal({ dateObj: slot.dateObj, logs })}
                        className={`relative aspect-[3/2] overflow-hidden p-3 border-b border-slate-100 cursor-pointer flex flex-col justify-end ${
                      ...
                      {/* Bottom Text Frame (3:4 ratio = exactly 2x height of the top frame) */}
                      <div className="p-3 aspect-[3/4] flex flex-col overflow-hidden">
                        {hasLog && pLog.pageContent && (
                          <p className="text-[11px] text-slate-500 leading-normal whitespace-pre-wrap line-clamp-10">
                            {pLog.pageContent}
                          </p>
                        )}
                      </div>
