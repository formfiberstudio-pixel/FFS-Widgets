...
const isOntarioStatHoliday = (dateObj) => {
  return getOntarioStatHolidayName(dateObj) !== null;
};
                      ...
                      {!isHidden && (
                        <div className="p-2 pt-0 space-y-1.5 border-t border-slate-150">
                          {projs.map((p, i) => {
                            ...
                            // Highlight override based on matrix hover mapping
                            if (isHovered) {
                            ...
                            {Array.from({ length: 37 }).map((_, colIndex) => {
                              ...
                              if (!isValidCalendarDay) {
                                return (
                                  <div 
                                    key={colIndex} 
                                    className={`w-5 h-5 z-10 rounded-xs ${isWeekendColumn ? 'bg-rose-50/30' : ''}`} 
                                  />
                                );
                              }
                              ...
                              // Dynamic Container Slot Tint
                              const slotBackground = isStatHoliday 
                                ? 'bg-amber-50/40' 
                                : isWeekend 
                                  ? 'bg-rose-50/40' 
                                  : '';
                              ...
                              return (
                                <div
                                  ...
                                  onMouseLeave={() => setHoveredProjectTitle(null)}
                                  className={`w-5 h-5 flex items-center justify-center relative z-10 cursor-pointer group/node rounded-xs ${slotBackground}`}
                                >
                                  <div
