...
const getOntarioStatHolidayName = (dateObj) => {
  ...
  // Civic Holiday: 1st Monday of August (Widely observed in Ontario)
...
function App() {
  ...
  const handleWeekClick = (mIdx, weekIndex) => {
    ...
    // Search for the first valid calendar day in this week segment
    let targetDay = null;
    for (let i = 0; i < 7; i++) {
                        ...
                        <div key={monthLabel} className="grid grid-cols-[65px_1fr] items-center relative group">
                          ...
                          <div className="grid grid-cols-37 gap-1 items-center relative py-1">
                            ...
                            {Array.from({ length: 37 }).map((_, colIndex) => {
                              ...
                              // Week block boundary rounding classes
                              const isWeekStart = colIndex % 7 === 0;
                              const isWeekEnd = colIndex % 7 === 6 || colIndex === 36;
                              const weekRoundClass = `${isWeekStart ? 'rounded-l-md' : ''} ${isWeekEnd ? 'rounded-r-md' : ''}`;
                              ...
                              if (!isValidCalendarDay) {
                                return (
                                  <div 
                                    ...
                                    className={`w-5 h-5 z-10 transition-colors cursor-pointer ${weekRoundClass} ${
                                      isHoveredWeekCell ? 'bg-rose-100/70' : isWeekendColumn ? 'bg-rose-50/30' : ''
                                    }`} 
                                  />
                                );
                              }
                              ...
                              // Check if this dot belongs to the currently hovered project
                              const isHoveredProject = hasLog && primaryLog && (primaryLog.Projects || 'Untitled Project') === hoveredProjectTitle;

                              // Dynamic Container Slot Tint & Week Hover Highlight
                              ...
                              if (hasLog) {
                                ...
                                // Apply overriding style when this dot matches the hovered project
                                if (isHoveredProject) {
                                  ringClass = 'ring-2 ring-rose-500/50 scale-125 z-20';
                                }
                              ...
                              return (
                                <div
                                  ...
                                  className={`w-5 h-5 flex items-center justify-center relative z-10 cursor-pointer group/node transition-colors ${weekRoundClass} ${slotBackground}`}
                                >
                                  <div
                                    onClick={(e) => {
                                      e.stopPropagation();
                                    ...
                                    className={`w-4 h-4 rounded-full flex items-center justify-center text-[8px] transition-all ${dotStyles} ${
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-xs" onClick={() => setSelectedLogModal(null)}>
          <div className="w-full max-w-2xl max-h-[80vh] bg-white rounded-xl flex flex-col overflow-hidden shadow-xl border border-slate-200" onClick={(e) => e.stopPropagation()}>
            <div className="px-5 py-3.5 border-b border-slate-100 flex items-center justify-between bg-slate-50">
              ...
              {/* Modal Title with Stat Holiday Name */}
