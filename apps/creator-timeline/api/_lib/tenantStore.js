...
// Family activations (see gumroad.js's FAMILY_BYPASS_KEY) aren't tracked in
// any separate registry -- this just scans the existing tenant records and
// filters to the ones activated with that key specifically
// (bypassKind === 'family'), for the admin panel's list/revoke view. The
...
export async function listFamilyTenants() {
  const keys = [];
  let cursor = '0';
  do {
  ...
  return keys
    ...
    .filter((entry) => entry.record?.bypassKind === 'family');
}
